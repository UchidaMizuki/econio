...
#'
#' @export
io_total_input <- function(data,
                           input_sector_type = c("industry", "import", "value_added"),
                           output_sector_type = "industry",
                           same_region = FALSE) {
  input_sector_type <- rlang::arg_match(input_sector_type, c("industry", "import", "value_added"),
                                        multiple = TRUE)
  output_sector_type <- rlang::arg_match(output_sector_type, c("industry", "final_demand", "export", "import"),
                                         multiple = TRUE)

  input <- data |>
    dplyr::filter(io_sector_type(.data$input) %in% input_sector_type,
                  io_sector_type(.data$output) %in% output_sector_type)
  if (same_region) {
    input <- dibble::broadcast(input * io_same_region(data),
...
io_total_output <- function(data,
  ...
  output <- data |>
    dplyr::filter(io_sector_type(.data$input) %in% input_sector_type,
                  io_sector_type(.data$output) %in% output_sector_type)
  if (same_region) {
    output <- dibble::broadcast(output * io_same_region(data),
