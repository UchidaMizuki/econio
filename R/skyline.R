io_tidy_skyline <- function(x, ...) {
  ...
  if (!inherits(x, "io_table_competitive_import")) {
    cli::cli_abort(
      "{.fn io_tidy_skyline} is not implemented for {.cls {class(x)}}."
    )
  }
  ...
  io_production_induce(x, endogenize_import = FALSE) |>
    ...
    dplyr::mutate(
      ...
      ymax_import_rate = .data$rate_self_sufficiency_rate +
        .data$rate_import_rate,

      yend_self_sufficiency_rate = dplyr::lead(
        .data$ymax_self_sufficiency_rate
      ),

      .keep = "unused"
    ) |>
    tidyr::pivot_longer(
      tidyselect::starts_with(c("rate", "ymin", "ymax", "yend")),
      names_to = c(".value", "rate_type"),
      names_pattern = "^(rate|ymin|ymax|yend)_(.*)",
      names_transform = list(
        rate_type = \(x) factor(x, c("import_rate", "self_sufficiency_rate"))
      )
    )
}
...
io_autolayer_skyline_rect <- function(object, ...) {
  data <- broom::tidy(object, type = "skyline")
...
io_autoplot_skyline <- function(object, ...) {
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::scale_y_continuous("Rate", labels = scales::label_percent()) +
    ggplot2::scale_fill_manual(
      "Rate type",
      values = c("self_sufficiency_rate" = "white", "import_rate" = "gray"),
      labels = c(
